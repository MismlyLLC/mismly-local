...
class OsTimeHelper {
	...
	public static function get_readable_date_from_string( string $date, string $input_timezone_name = '', string $output_timezone_name = '' ): string {
		...
		return self::get_readable_date( $date_obj, $output_timezone_name );
			...
			// Modifiqué esta funcion para que primero verifique si existe la cookie de la zona horaria del cliente y si no existe, use la zona horaria de WordPress
	public static function get_wp_timezone() {
		if ( self::$timezone ) {
			return self::$timezone;
		}
		try {
			// Si existe la cookie de la zona horaria del cliente, usarla
			if ( isset($_COOKIE['client_timezone']) ) {
				$client_tz = sanitize_text_field($_COOKIE['client_timezone']);
				error_log("✅ Cookie client_timezone detectada: " . $client_tz);
				self::$timezone = new DateTimeZone($client_tz);
				return self::$timezone;
			}

			// Si no, usar la zona horaria de WordPress
