...
class OsBookingModel extends OsModel {
	...
	public function get_nice_created_at( $include_time = true ) {
		...
		$client_timezone = isset($_COOKIE['client_timezone']) ? sanitize_text_field($_COOKIE['client_timezone']) : null;
		$timezone = $client_timezone ? new DateTimeZone($client_timezone) : OsTimeHelper::get_wp_timezone();
		$wp_timezone_date = $utc_date->setTimezone($timezone);
	...
	public function get_nice_start_date_for_customer($customer_timezone = false, $hide_year = false) {
		if (!$customer_timezone) {
			$customer_timezone = isset($_COOKIE['client_timezone']) ? sanitize_text_field($_COOKIE['client_timezone']) : $this->get_customer_timezone();
		}
